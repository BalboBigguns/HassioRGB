...
void handleMQTTMessage(char* p_topic, byte* p_payload, unsigned int p_length) {
  ...
  if (String(MQTT_COMMAND_TOPIC).equals(p_topic)) {
    DeserializationError err = deserializeJson(staticJsonDocument, p_payload);
    ...
    if (root.containsKey("color")) {
      ...
      if (led.setColor(r, g, b)) {
        DEBUG_PRINT(F("INFO: Color changed to: "));
        DEBUG_PRINT(led.getColor().red);
        DEBUG_PRINT(F(", "));
        DEBUG_PRINT(led.getColor().green);
        DEBUG_PRINT(F(", "));
        DEBUG_PRINTLN(led.getColor().blue);
      }
    }

    if (root.containsKey("brightness")) {
      if (led.setBrightness(root["brightness"])) {
        DEBUG_PRINT(F("INFO: Brightness changed to: "));
        DEBUG_PRINTLN(led.getBrightness());
      }
    }
    ...
    if (root.containsKey("color_temp")) {
      ...
      if (led.setColorTemperature(root["color_temp"])) {
        DEBUG_PRINT(F("INFO: Color temperature changed to: "));
        DEBUG_PRINTLN(led.getColorTemperature());
      }
    }
    
    if (root.containsKey("effect")) {
      const char* effect = root["effect"];
      if (led.setEffect(effect)) {
