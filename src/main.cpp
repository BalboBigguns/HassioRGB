...
void handleMQTTMessage(char *p_topic, byte *p_payload, unsigned int p_length)
{
    ...
    if (String(MQTT_COMMAND_TOPIC).equals(p_topic)) {
        ...
        if (!value.isNull()) {
            if (led.setBrightness(value)) {
                DEBUG_PRINT(F("INFO: Brightness changed to: "));
                DEBUG_PRINTLN(led.getBrightness());
            }
        }

        value = root["color_temp"];
        if (!value.isNull()) {
            // stops the possible current effect
            led.setEffect(EFFECT_NOT_DEFINED_NAME);

            if (led.setColorTemperature(value)) {
        ...
        value = root["effect"];
        if (!value.isNull()) {
            if (led.setEffect(value)) {
                DEBUG_PRINT(F("INFO: Effect started: "));
                DEBUG_PRINTLN(value.as<String>());
            }
        }
    }
}
